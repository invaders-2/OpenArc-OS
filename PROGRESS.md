...
23. trusted death proof 只来自同一 supervisor lifetime 的真实 `child.on("exit")`，或上一次 production supervisor 真实观测后持久化的 trusted `EXITED`（reason 白名单 `SUPERVISOR_OBSERVED_EXIT` / `EXECUTOR_SPAWN_FAILED`）；非 trusted reason 的 EXITED 一律 fail closed。
