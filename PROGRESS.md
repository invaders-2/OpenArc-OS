...
| test:d4-03c4-closure | **15 / 15 PASS**（tri-state probe / fail-closed rehydrate / disk-backed UNKNOWN + late-effect safety） |
...
20. **Probe failure is not death proof.** Liveness probe 恒为 tri-state `ALIVE / DEFINITELY_GONE / UNKNOWN`；只有 `DEFINITELY_GONE`（白名单当前仅 `ENOENT`）允许 `observeExit`；timeout / 任意 socket error / probe 异常一律 UNKNOWN → fail closed。
21. `ECONNREFUSED` 不是 death proof（无法跨平台证明它不是 timeout / backlog / 权限 / 资源耗尽的别名）；宁可 fail closed，不追求自动 recovery 成功率。
22. `rehydrate()` 的 UNKNOWN 必须单独计数、persisted executor record 保持 `ACTIVE`，绝不改写成 `EXITED` / `OS_EXECUTOR_ENDPOINT_ABSENT`。
...
**已由 D4-03C4 Closure 关闭**：cold-restart runtime quiescence probe 必须 fail closed（tri-state liveness；probe uncertainty / timeout / 任意 socket error != death proof）。
其余项**不因 D4-03C4 / Closure 关闭**。

## 5. D4-03C 准入

D4-03C1 = **PASS**；**D4-03C2 = PASS**；**D4-03C3 = PASS candidate**、**D4-03C3 Closure = PASS candidate**；**D4-03C4 = PASS candidate**、**D4-03C4 Closure = PASS candidate**（cold-restart quiescence probe fail closed；tri-state liveness）、**D4-03C overall = PASS candidate**；**D4-03D = NOT STARTED**，由 ChatGPT 审计后决定，禁止自动进入。
