...
| **D4-03C2 Controlled Reversible Write** | **PASS candidate** | 第一条真实 production REVERSIBLE_WRITE：**resource.trash → ResourceService.delete**；contract 显式 executionPolicy=CONTROLLED_REVERSIBLE_WRITE（其余 write tool 仍 WRITE_EXECUTION_DISABLED）；真实 Domain plan（只读，expectedVersion/expectedEffects 来自 Domain）；Approval+Lease 必须；**原子 claim LEASED→RUNNING 后才 dispatch**（唯一 executor）；Domain invocation exactly 1 / business mutation exactly 1；业务乐观并发（mutation transaction 内 re-read + expectedVersion）→ VERSION_CONFLICT = SIDE_EFFECT_PRECONDITION_CHANGED（0 mutation）；**Execution != Verified Effect**：真实 Domain verifier PASS 才 SUCCEEDED；verifier 不可判 → UNKNOWN_EFFECT + Task/Step BLOCKED（0 retry）；lease SUCCEEDED 后 RELEASED；真实 ResourceService.restore 证明可逆；schema 保持 v13；test:d4-03c2 **30/30** |
...
D4-03C1 = **PASS**（合同层）；**D4-03C2 = PASS candidate**（1 条受控真实 REVERSIBLE_WRITE，resource.trash）；**D4-03C overall = PARTIAL**。**D4-03C3 = 未开始**，由 ChatGPT 审计后决定，禁止自动进入。C2 只开放 resource.trash 一条 REVERSIBLE_WRITE：IRREVERSIBLE_WRITE / EXTERNAL_SIDE_EFFECT / PRIVILEGED / Shell / Terminal / filesystem generic mutation / MCP / Browser automation / Device Agent mutation / Canvas mutation / App Center mutation / Adobe control 仍全部 BLOCKED；Final Approval UI 仍 NOT IMPLEMENTED。
