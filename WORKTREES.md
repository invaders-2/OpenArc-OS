...
## 4. 视觉定标流程（本轮用 5 次往返换来的）

**"看起来正不正 / 偏不偏"类问题，不要靠几何推算，也不要靠浏览器预览量。**

代价实例（原生红绿灯在 38px 顶栏里的垂直位置）：
```
y=16  最初
y=13  按几何居中((38-12)/2) → 用户判"偏上"（后证实那一眼看的是旧实例）
y=15  往下 2px → "再往下"
y=17  再往下   → 用户截图实测：彩点中心比图标**低 5px**（矫枉过正）
y=12  用截图反推 5px 偏差 → 居中 ✅
```
教训与流程：
1. **要一张用户真实窗口的截图**（工具每次运行都会把焦点抢回 harness，自己 `screencapture` 抓到的是 harness 的窗口 —— 本轮前两次"实拍"都抓错了）。
2. 在截图上**逐像素量**两个参照物的中心差（注意 2x/Retina 缩放：设备像素 ÷2 = CSS px）。
3. 用"当前参数 + 实测偏差"**反推**目标值，一次改到位。
4. 改完**必看**：参数是"载荷级"可核对的（`grep` 产物里的值），但"看起来对不对"只能由用户确认。
5. 顺带记住：原生窗口在 macOS 上会**缓存/沿用旧实例** —— 判定前先 `pkill` 冷启动，否则会拿旧画面下结论。
