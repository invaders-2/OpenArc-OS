// D4-03C1 标准入口。npm run test:d4-03c1
...
const ART = path.join(ROOT, "artifacts", "d4-03c1");
...
const report = { ...summary, files: FILES, versions, mode: "SIDE-EFFECT AUTHORITY (no write execution)", machine: { os: process.platform + " " + os.release(), arch: process.arch, node: process.version }, at: new Date().toISOString() };
fs.writeFileSync(path.join(ART, "d4-03c1-gate.json"), JSON.stringify(report, null, 2));
