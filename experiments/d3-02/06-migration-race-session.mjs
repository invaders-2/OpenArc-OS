...
// 1. 全新库 → 当前 schema（D3-04A 起为 v4：v2 授权表 + v3 设备表 + v4 资源存储表都要在）
{
  ...
  try {
    ...
    const v4Tables = ["library_resources", "content_objects", "resource_versions", "resource_relations", "resource_import_jobs"];
    const hasV4 = v4Tables.every((t) => !!s.connection.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name=?").get(t));
    p.assert("全新库 schemaVersion = 当前版本（v2 授权 + v3 设备 + v4 资源存储）", s.schemaVersion === SCHEMA_VERSION && hasV3 && hasV4, String(s.schemaVersion) + " v3tables=" + hasV3 + " v4tables=" + hasV4);
...
// 2. v1 → v2 迁移
{
  ...
  try {
    ...
    p.assert("v1 → 当前 schema 后 login/lock/unlock 成立", s.schemaVersion === SCHEMA_VERSION && login.ok && lock.ok && unlocked.ok, "");
    s.close();
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
