// D4-03C1 Closure 标准入口。npm run test:d4-03c1-closure
...
const ART = path.join(ROOT, "artifacts", "d4-03c1-closure");
...
const report = { ...summary, files: FILES, versions, mode: "C1 CLOSURE (real contention + persisted restart; 0 write)", machine: { os: process.platform + " " + os.release(), arch: process.arch, node: process.version }, at: new Date().toISOString() };
fs.writeFileSync(path.join(ART, "d4-03c1-closure-gate.json"), JSON.stringify(report, null, 2));
