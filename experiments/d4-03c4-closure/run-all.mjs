// D4-03C4 Closure 标准入口。npm run test:d4-03c4-closure
// 唯一主题：Cold-restart Runtime Quiescence Probe must fail closed。
...
const ART = path.join(ROOT, "artifacts", "d4-03c4-closure");
fs.rmSync(ART, { recursive: true, force: true });
fs.mkdirSync(ART, { recursive: true });

const FILES = ["tests/side-effect-c4-closure.test.mjs"];
...
try {
  ...
  versions = { schemaVersion: store.SCHEMA_VERSION, tools: new ToolRegistry().ids(), liveness: supervisor.LIVENESS, definitelyGoneErrnos: [...supervisor.DEFINITELY_GONE_ERRNOS] };
...
const report = {
  ...summary,
  files: FILES,
  mode: "C4 CLOSURE (tri-state liveness probe: ALIVE / DEFINITELY_GONE / UNKNOWN; probe failure is not death proof; UNKNOWN never observeExit and never quiesced)",
  contract: { onlyDefinitelyGoneMayObserveExit: true, unknownFailsClosed: true, schemaBumped: false },
  versions,
  machine: { os: process.platform + " " + os.release(), arch: process.arch, node: process.version },
  at: new Date().toISOString(),
};
fs.writeFileSync(path.join(ART, "d4-03c4-closure-gate.json"), JSON.stringify(report, null, 2));
