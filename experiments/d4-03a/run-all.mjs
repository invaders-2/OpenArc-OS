// D4-03A 标准入口。npm run test:d4-03a
...
const ART = path.join(ROOT, "artifacts", "d4-03a");
fs.rmSync(ART, { recursive: true, force: true });
fs.mkdirSync(ART, { recursive: true });
const FILES = [
  "tests/tool-registry.test.mjs",
  "tests/tool-proposal.test.mjs",
  "tests/tool-authorization.test.mjs",
  "tests/tool-gate-security.test.mjs",
  "tests/tool-proposal-e2e.test.mjs",
...
const report = { ...summary, files: FILES, versions, execution: "NOT_EXECUTED", machine: { os: process.platform + " " + os.release(), arch: process.arch, node: process.version }, at: new Date().toISOString() };
fs.writeFileSync(path.join(ART, "d4-03a-gate.json"), JSON.stringify(report, null, 2));
