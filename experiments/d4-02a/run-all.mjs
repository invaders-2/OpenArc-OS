// D4-02A 标准入口。npm run test:d4-02a
import { spawn } from "node:child_process";
import path from "node:path";
import fs from "node:fs";
const HERE = import.meta.dirname;
const ROOT = path.resolve(HERE, "../..");
const ART = path.join(ROOT, "artifacts", "d4-02a");
fs.rmSync(ART, { recursive: true, force: true });
fs.mkdirSync(ART, { recursive: true });
const FILES = ["tests/task-domain.test.mjs", "tests/task-store.test.mjs", "tests/task-recovery.test.mjs"];
// --test-concurrency=1：performance smoke 的基线不能被并行文件污染
...
fs.writeFileSync(path.join(ART, "d4-02a-gate.json"), JSON.stringify({ ...summary, files: FILES, at: new Date().toISOString() }, null, 2));
