// D4-03C2 Closure-3 标准入口。npm run test:d4-03c2-closure3
...
const ART = path.join(ROOT, "artifacts", "d4-03c2-closure3");
...
const report = { ...summary, files: FILES, versions, mode: "C2 CLOSURE-3 (runtime identity final seal: no acquire/recovery override, runtime-aware duplicate, cross-runtime same-holder conflict)", machine: { os: process.platform + " " + os.release(), arch: process.arch, node: process.version }, at: new Date().toISOString() };
fs.writeFileSync(path.join(ART, "d4-03c2-closure3-gate.json"), JSON.stringify(report, null, 2));
