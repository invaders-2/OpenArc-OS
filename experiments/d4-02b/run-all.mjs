// D4-02B 标准入口。npm run test:d4-02b
...
const ART = path.join(ROOT, "artifacts", "d4-02b");
fs.rmSync(ART, { recursive: true, force: true });
fs.mkdirSync(ART, { recursive: true });
const FILES = [
  "tests/harness-acp-protocol.test.mjs",
  "tests/harness-acp-isolation.test.mjs",
  "tests/harness-acp-cancel.test.mjs",
  "tests/harness-acp-lifecycle.test.mjs",
  "tests/harness-acp-permission.test.mjs",
];
let versions = {};
try {
...
fs.writeFileSync(path.join(ART, "d4-02b-gate.json"), JSON.stringify(report, null, 2));
