// D4-03C3 标准入口。npm run test:d4-03c3
...
const ART = path.join(ROOT, "artifacts", "d4-03c3");
...
const report = { ...summary, files: FILES, versions, mode: "C3 AMBIGUOUS RESULT (real Domain recovery verifier / quiescence gate / crash matrix / idempotency across restart)", machine: { os: process.platform + " " + os.release(), arch: process.arch, node: process.version }, at: new Date().toISOString() };
fs.writeFileSync(path.join(ART, "d4-03c3-gate.json"), JSON.stringify(report, null, 2));
