...
- **状态**：**D4-02A = PASS**；**D4-02B ACP Harness Adapter = PASS**；**D4-02C Task ↔ Harness Orchestration = PASS**（macOS）；**D4-02 macOS Task/Harness Core = PASS**（cross-platform = PARTIAL，Windows NOT VERIFIED）；**D4-03 Controlled Tool Proxy = BLOCK**
...
## Store Schema（D4-02A = schema v9；D4-02C = schema v10）

D4-02A：`tasks` / `task_steps` / `task_model_calls` / `task_events` + 索引：
...
D4-02A：`SCHEMA_VERSION = 9`（v8 之上只加 Task Runtime 表）。D4-02C：`SCHEMA_VERSION = 10`（v9 之上加 `task_harness_runs / task_artifacts / task_verifications`）。`v1→current … v9→current` 与 v10 级失败整级回滚（`user_version` 保持 9、v10 表不残留）全部 PASS。
...
D4-02A 已交付持久权威。**D4-02B ACP Harness Adapter = PASS**，冻结条件：ACP ONLY；`HARNESS_RAW_PROVIDER_KEY = FORBIDDEN`；Harness 只与 Model Proxy 通信，只接收 Proxy endpoint + scoped capability + safe model snapshot；不拥有 persistent queue/state/retry/tool/lease/permission authority。

## D4-02C Orchestration Boundary（已交付）

**D4-02C Task ↔ Harness Orchestration = PASS。** TaskHarnessOrchestrator 只做编排，不是第二 authority：所有持久 mutation 经 `TaskService`，绝不直写 SQLite。ACP event 经显式 mapping 落 safe TaskEvent；Harness event 不能改 Task/Step status；终态只由 Orchestrator 依据 stopReason / cancel state / errors / revision 决定。

新增 schema v10：`task_harness_runs`（status `STARTING/RUNNING/SUCCEEDED/BLOCKED/CANCELLED/FAILED`）、`task_artifacts`（text/json）、`task_verifications`（EXACT_TEXT/SCHEMA_VALID）。Artifact + Verification + Step + Task + Events **同一事务**提交；成功必须有 Verification PASS。Cancel 两阶段（persist `cancel_requested` → ACP cancel → revoke → finalize）保证 race 唯一解。crash/timeout/restart → `BLOCKED`/`RECOVERY_REQUIRED`/`unknown_effect`，0 respawn / 0 replay。tool proposal 0 execute；permission 一律 reject；`mcpServers = []`。capability 加 executionBinding 且 turn 结束立即 revoke。

**D4-03 Controlled Tool Proxy = BLOCK**，直到 D4-02 自身 PASS；D4-03 不自动开始。
