...
## 23. 状态更新：由 D2-02 关闭的缺口（**不改写上文历史**）

本节只做**追加标注**，不修改第 22 节的任何原始判定 —— 第 22 节的 PARTIAL
是 D2-01 当时的真实结论，必须留在那里作为历史。

| D2-01 §22 列出的缺口 | 现在 | 依据 |
|----------------------|------|------|
| ① 桌面端 12 个组件只有视觉契约，尚未组件化 | **CLOSED BY D2-02** | 12 个 Component 已真正拆分；`Window` 不拥有业务状态；且**由产品自身消费**（不是只在 gallery 里存在） |
| ② 对话框焦点陷阱 + 焦点返回 NOT VERIFIED | **CLOSED BY D2-02** | `src/desktop/Dialog.tsx` 提供焦点陷阱 / `aria-modal` / Esc / 关闭后焦点归还原触发元素；`experiments/d2-02/dialog-a11y.mjs` 33/33；产品有两条真实消费路径（文件夹删除确认、控制中心确认） |
| ③ Windows 视觉保真度 NOT VERIFIED | **仍未关闭** | 无 Windows 真机；D2-02 同样记 NOT VERIFIED，不得外推 |
| ④ `styles.css` 迁移欠账 | **部分关闭** | D2-02 触碰到的组件已迁到 token（迁 14 处、保留 4 处并逐条注明理由），未触碰部分保持原状 —— 见 D2-02 §30 的范围约束 |

「组件由产品自身消费」这一条是 D2-01 缺口 ① 真正的验收条件：
组件存在但没人用等于没做。D2-02 之后，桌面、窗口、Dock、顶栏、
右键菜单、对话框全部走同一套组件与同一条 WindowCommand 层。

其它缺口（Windows）与 D2-02 的未验证项合并记录在
`docs/decisions/D2-02-window-system.md` §43。
