...
管理员管理账号、角色、设备加入、团队资源和共享技能。成员只访问分配给自己的项目、应用、工具与设备。个人技能默认私有，用户可明确分享到团队；团队技能由管理员控制发布权限。

局域网服务负责身份、项目和任务协调，各设备执行端负责调用本机应用与工具。任务执行前明确目标设备与所用账号；设备离线时保留可见状态，不默默切换设备。授权按用户、设备和工具限制，安装或启用 Skill 不扩大权限。记录谁在何时、在哪台设备调用了什么工具及执行结果。
...
两个局域网账号在授权设备上共享项目并分配任务；AI 使用一个真实 MCP 执行动作且结果可验证；用户可安装或自建 Skill、试运行、编辑、导出导入、启停和分享；无权调用、连接失败和设备离线均有明确反馈。界面原型验收与真实功能验收分别记录。
