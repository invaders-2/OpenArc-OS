...
test("v4 -> v5：升级成功，v5 表建立，identity/authorization/resource 数据完整", async () => {
  ...
  assert.equal(reopened.identity.schemaVersion, 6);
  ...
  for (const t of [...V5_TABLES, ...V6_TABLES]) assert.equal(hasTable(reopened.identity.connection, t), true, t + " 应存在");
...
test("v5 级迁移失败 -> user_version 停在 4，v5 表不残留，旧 Resource 仍可读", async () => {
  ...
  const raw = new DatabaseSync(dbPath);
  assert.equal(raw.prepare("PRAGMA user_version").get().user_version, 4);
  for (const t of [...V5_TABLES, ...V6_TABLES]) assert.equal(hasTable(raw, t), false, t + " 不应残留");
  ...
  assert.equal(repaired.identity.schemaVersion, 6);
