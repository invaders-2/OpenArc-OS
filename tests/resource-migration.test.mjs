...
async function makeV3Db() {
  ...
  for (const table of [...V12_TABLES, ...V13_TABLES]) raw.exec("DROP TABLE IF EXISTS " + table);
...
test("v3 -> v4：升级成功且 identity / department / device 数据完整", () => {
  return (async () => {
    ...
    for (const table of [...V4_TABLES, ...V5_TABLES, ...V6_TABLES, ...V7_TABLES, ...V8_TABLES, ...V9_TABLES, ...V10_TABLES, ...V11_TABLES, ...V12_TABLES, ...V13_TABLES]) assert.equal(hasTable(reopened.identity.connection, table), true, table + " 应存在");
