...
test("v10 -> v11：task_tool_proposals / tool_decisions 建立，旧数据完整", async () => {
  ...
  try {
    ...
    for (const t of V11_TABLES) assert.equal(hasTable(reopened.identity.connection, t), true, t + " 应存在");
    assert.equal(hasTable(reopened.identity.connection, "tool_executions"), false, "D4-03A 不应有 tool_executions");
...
test("v11 迁移失败 -> 整级回滚：user_version 停 10，v11 表不残留", async () => {
  ...
  try {
    ...
    assert.equal(repaired.identity.schemaVersion, 11);
    for (const t of V11_TABLES) assert.equal(hasTable(repaired.identity.connection, t), true, t + " 修复后应存在");
