/**
 * D3-01 UI 验收：拉起**真实 Electron**，驱动产品真实页面
 * （dist/index.html + electron/preload.cjs）走完
 *   首次初始化 → 登录 → 重启恢复 → 锁定 → 错误解锁 → 正确解锁 → 登出 → 重启不复活。
 *
 * 不依赖 Playwright：本环境 Playwright 1.55 与 Electron 内置 Chromium 的 CDP 握手超时，
 * 因此改用主进程自持窗口 + webContents.executeJavaScript 驱动（§48 允许的替代路径，
 * 与 tests/native-view.mjs 同一手法）。
 *
 * 本机沙箱起不来时传：
...
const probeDir = path.join(here, "fixtures", "identity-ui-probe");
...
console.log("PASS: 首次初始化 / 登录 / 重启恢复 / 锁定 / 解锁 / 登出 在真实 Electron 中全部成立。");
