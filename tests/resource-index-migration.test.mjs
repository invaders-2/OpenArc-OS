...
async function makeV5Db() {
  ...
  for (const t of [...V12_TABLES, ...V11_TABLES, ...V10_TABLES, ...V9_TABLES, ...V6_TABLES, ...V7_TABLES, ...V8_TABLES]) raw.exec("DROP TABLE IF EXISTS " + t);
...
test("v5 -> v6：派生表建立，FTS 为 virtual table，资源可重新索引并搜索", () => {
  return (async () => {
    ...
    for (const t of [...V6_TABLES, ...V7_TABLES, ...V8_TABLES, ...V9_TABLES, ...V10_TABLES, ...V11_TABLES, ...V12_TABLES]) assert.equal(hasTable(reopened.identity.connection, t), true, t + " 应存在");
