...
test("v9 -> v10：HarnessRun / Artifact / Verification 表建立，旧数据完整", async () => {
  ...
  try {
    ...
    for (const t of V10_TABLES) assert.equal(hasTable(reopened.identity.connection, t), true, t + " 应存在");
...
test("v10 迁移失败 -> 整级回滚：user_version 停 9，v10 表不残留", async () => {
  ...
  try {
    ...
    assert.equal(repaired.identity.schemaVersion, 10);
    for (const t of V10_TABLES) assert.equal(hasTable(repaired.identity.connection, t), true, t + " 修复后应存在");
