 ...
 *
 * crashPoint:
 *   · before_mutation → mutation 未发生就退出（NOT_APPLIED 场景）；
 *   · after_mutation  → mutation 已提交后退出（late APPLIED 场景）。
...
bundle = createSideEffectExecutorBundle({
  ...
  resourceServiceWrapper: (svc) => {
    ...
    svc.delete = async (a) => {
      if (args.crashPoint === "after_mutation") { const out = await real(a); await exitAfter("after_mutation"); return out; }
      await exitAfter("before_mutation");
