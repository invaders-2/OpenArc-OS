...
const SIDE_EFFECT_ERROR = Object.freeze({
  ...
  // D4-03C4 approval gateway：等待超时 / 重启后放弃，一律 0 execute。
  APPROVAL_TIMEOUT: "SIDE_EFFECT_APPROVAL_TIMEOUT",
  APPROVAL_ABANDONED: "SIDE_EFFECT_APPROVAL_ABANDONED",
...
function evaluateExecutionEligibility(snapshot = {}) {
  ...
  // D4-03C2 Closure：execution ownership = callId + exact leaseId + holderId + runtime instance。
  if (s.leaseId != null && lease.lease_id !== s.leaseId) return fail(ELIGIBILITY.DENIED, SIDE_EFFECT_ERROR.LEASE_NOT_HELD);
